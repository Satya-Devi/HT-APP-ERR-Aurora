...
import { fetchJobsData , deleteDraft} from "@/app/overview/action";
...
const DraftTable = ({   showPagination }: Props) => {
  ...
  const handleDiscardDraft = async (draftId: string | undefined) => {
    try {
      await deleteDraft(draftId);
      // Refresh the drafts list
      getDrafts(activePage); // Assuming you have a function to fetch drafts
    } catch (error) {
    ...
    const getDrafts = async (page: any | undefined) => {
      try {
        setIsLoading(true);
        const result = await fetchJobsData({limit:20, step:page});
      ...
      <Table.Tr key={index}>
        ...
        <Table.Td style={{ textAlign: "center" }}>
          ...
          >
            <div>
              <Button
               onClick={() => handleDiscardDraft(job.id)}
                size="xs"
                style={{
